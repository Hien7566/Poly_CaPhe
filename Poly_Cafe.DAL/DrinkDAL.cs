...
namespace Poly_Cafe.DAL
{
    public class DrinkDAL
    {
        ...
        public List<DrinkDTO> GetAll()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                list.Add(new DrinkDTO
                {
                    ...
                    Price = Convert.ToDecimal(row["price"]), // Chuyển qua Decimal an toàn cho tiền tệ
        ...
        {
            List<DrinkDTO> list = new List<DrinkDTO>();
            string sql = "SELECT * FROM drinks WHERE category_id = @p0 AND active = 1 ORDER BY id DESC";
